...
public abstract class BeanUtils {
	...
	public static <T> T instantiateClass(Constructor<T> ctor, Object... args) throws BeanInstantiationException {
		...
		try {
			...
			throw new BeanInstantiationException(ctor, "Is it an abstract class?(它是抽象类吗?)", ex);
		}
		catch (IllegalAccessException ex) {
			throw new BeanInstantiationException(ctor, "Is the constructor accessible?(构造函数可访问吗?)", ex);
		}
		catch (IllegalArgumentException ex) {
			throw new BeanInstantiationException(ctor, "Illegal arguments for constructor(构造函数的非法参数)", ex);
		}
		catch (InvocationTargetException ex) {
			throw new BeanInstantiationException(ctor, "Constructor threw exception(构造函数把例外)", ex.getTargetException());
		}
	}
